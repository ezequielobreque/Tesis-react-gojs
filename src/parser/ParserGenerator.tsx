...
import { onInputEditorContentChange } from "./errors";
...
export const ParserGenerator = ({ changeStateCallback, nodeData }: any) => {
  ...
  let action: ActionsFunctionsClass = useMemo(
    () => new ActionsFunctionsClass(),
    []
  );
  ...
  const handleSubmit = (
    lexAndParseResult: any,
    instance: CodeMirror.Editor
  ) => {
    try {
      console.log(
        "🚀 ~ file: ParserGenerator.tsx ~ line 27 ~ handleSubmit ~ ñexAndParseResult.lexErrors.length",
        lexAndParseResult.lexErrors.length
      );
      assert.strictEqual(lexAndParseResult.lexErrors.length, 0);
      console.log(
        "🚀 ~ file: ParserGenerator.tsx ~ line 29 ~ handleSubmit ~ lexAndParseResult.parseErrors",
        lexAndParseResult.parseErrors
      );

      assert.strictEqual(lexAndParseResult.parseErrors.length, 0);
      console.log(
        "🚀 ~ file: ParserGenerator.tsx ~ line 32 ~ handleSubmit ~ lexAndParseResult",
        lexAndParseResult
      );
    } catch (e) {
      console.log(e);
    ...
    if (lexAndParseResult.cst) {
      const result = action.ActionsFunctions(state);
      if (result) {
